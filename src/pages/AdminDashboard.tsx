...
export default function AdminDashboard() {
  ...
  // Métricas del dashboard (con fallback a datos de ejemplo mientras carga el backend)
  const metricas = {
    totalEstudiantes: adminData.metrics.totalEstudiantes || 2847,
    totalDocentes: adminData.metrics.totalDocentes || 156,
    solicitudesPendientes: adminData.metrics.solicitudesPendientes || 23,
    certificadosGenerados: adminData.metrics.certificadosGenerados || 145,
    ingresosMensuales: adminData.metrics.ingresosMensuales || 328500,
    cursosActivos: adminData.metrics.cursosActivos || 87,
    usuariosActivos: adminData.metrics.usuariosActivos || 1234,
    satisfaccion: adminData.metrics.satisfaccion || 4.6
  };
  ...
  const handleSearch = (query: string) => {
    ...
    usuarios.filter(u => u.rol === 'estudiante').forEach(estudiante => {
      if (estudiante.nombre.toLowerCase().includes(searchLower) ||
          estudiante.email.toLowerCase().includes(searchLower)) {
        results.push({
          id: estudiante.id,
          tipo: 'estudiante',
          titulo: estudiante.nombre,
          subtitulo: estudiante.email,
          icono: 'estudiante',
          url: `/admin/estudiantes/${estudiante.id}`
        });
      }
    });
    ...
    usuarios.filter(u => u.rol === 'docente').forEach(docente => {
      if (docente.nombre.toLowerCase().includes(searchLower) ||
          docente.email.toLowerCase().includes(searchLower)) {
        results.push({
          id: docente.id,
          tipo: 'docente',
          titulo: docente.nombre,
          subtitulo: docente.email,
          icono: 'docente',
    ...
    cursos.forEach(curso => {
      if (curso.nombre.toLowerCase().includes(searchLower) ||
          curso.codigo.toLowerCase().includes(searchLower)) {
        results.push({
          id: curso.id,
          tipo: 'curso',
          titulo: curso.nombre,
          subtitulo: `Código: ${curso.codigo}`,
          icono: 'curso',
          url: `/admin/cursos/${curso.id}`
        });
      }
    });
    ...
    activos.forEach(activo => {
      if (activo.nombre.toLowerCase().includes(searchLower) ||
          activo.codigo.toLowerCase().includes(searchLower)) {
        results.push({
          id: activo.id,
          tipo: 'activo',
          titulo: activo.nombre,
          subtitulo: `${activo.codigo} - ${activo.ubicacion}`,
          icono: 'activo',
          url: `/admin/activos/${activo.id}`
        });
      }
    });
    ...
    empleados.forEach(empleado => {
      if (empleado.nombre.toLowerCase().includes(searchLower) ||
          empleado.identificacion.includes(searchLower)) {
        results.push({
          id: empleado.id,
          tipo: 'empleado',
          titulo: empleado.nombre,
          subtitulo: `${empleado.cargo} - ${empleado.departamento}`,
          icono: 'empleado',
          url: `/admin/recursoshumanos/${empleado.id}`
        });
      }
    });
    ...
    libros.forEach(libro => {
      if (libro.titulo.toLowerCase().includes(searchLower) ||
          ...
          libro.isbn.includes(searchLower)) {
        results.push({
          id: libro.id,
          tipo: 'libro',
          titulo: libro.titulo,
          subtitulo: `${libro.autor} - ISBN: ${libro.isbn}`,
          icono: 'libro',
          url: `/admin/biblioteca/${libro.id}`
        });
      }
    });
  ...
  const menuItems = [
    ...
    { id: 'anuncios', icon: Megaphone, label: 'Anuncios', color: 'text-orange-600' },
    { id: 'mensajesmasivos', icon: Send, label: 'Mensajes Masivos', color: 'text-blue-600' },
    ...
    { id: 'activos', icon: Building2, label: 'Gestión de Activos', color: 'text-orange-600' },
    { id: 'inventario', icon: ClipboardCheck, label: 'Inventario', color: 'text-purple-600' },
    { id: 'nomina', icon: CreditCard, label: 'Nómina', color: 'text-green-600' },
    { id: 'recursoshumanos', icon: Users, label: 'Recursos Humanos', color: 'text-blue-600' },
    { id: 'biblioteca', icon: BookOpen, label: 'Biblioteca', color: 'text-pink-600' },
    { id: 'carnetizacion', icon: User, label: 'Carnetización', color: 'text-purple-600' },
    { id: 'finanzas', icon: DollarSign, label: 'Finanzas y Contabilidad', color: 'text-green-600' },
    { id: 'reportes', icon: PieChart, label: 'Reportes', color: 'text-purple-600' },
    { id: 'analiticas', icon: BarChart3, label: 'Analíticas', color: 'text-indigo-600' },
  ...
  const renderizarCertificado = (plantilla: any, datosEstudiante: any = {}) => {
  ...
  const marcarComoLeido = (id: any) => {
    setMensajes(prev => prev.map(m => 
      m.id === id ? { ...m, leido: true } as any : m
    ));
  ...
  const renderMensajes = () => <MensajesAdminSection darkMode={darkMode} />;
  ...
  const renderFinanzas = () => {
    return <FinanzasContabilidadSection />;
  };
  ...
  const renderConfiguracion = () => (
    <div className="space-y-6">
      ...
      <div className={`${card} rounded-lg shadow p-6`}>
        <h3 className={`text-lg font-bold ${text} mb-4`}>Configuración de la Institución</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="text"
              value={configuracionInstitucion.nombre}
              className={`w-full px-3 py-2 border ${border} rounded-lg ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-900'}`}
              readOnly
            />
          </div>
          <div>
            ...
            <input
              type="text"
              value={configuracionInstitucion.nit}
              className={`w-full px-3 py-2 border ${border} rounded-lg ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-900'}`}
              readOnly
            />
          </div>
          <div>
            ...
            <input
              type="email"
              value={configuracionInstitucion.email}
              className={`w-full px-3 py-2 border ${border} rounded-lg ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-900'}`}
              readOnly
            />
          </div>
          <div>
            ...
            <input
              type="text"
              value={configuracionInstitucion.telefono}
              className={`w-full px-3 py-2 border ${border} rounded-lg ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-gray-900'}`}
              readOnly
            />
          </div>
      ...
      {/* Roles y Permisos */}
      <div className={`${card} rounded-lg shadow p-6`}>
        <h3 className={`text-lg font-bold ${text} mb-4`}>Roles y Permisos</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {roles.map(rol => (
            <div key={rol.id} className={`p-4 border ${border} rounded-lg`}>
              <div className={`w-12 h-12 ${rol.color} rounded-full flex items-center justify-center text-white font-bold mb-3`}>
                {rol.nombre.charAt(0)}
              </div>
              <h4 className={`font-bold ${text}`}>{rol.nombre}</h4>
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mb-2`}>{rol.descripcion}</p>
      ...
      <div className={`${card} rounded-lg shadow p-6`}>
        <h3 className={`text-lg font-bold ${text} mb-4`}>Configuración de Pago</h3>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className={darkMode ? 'text-gray-300' : 'text-gray-700'}>Pasarela de Pago:</span>
            <span className={`font-semibold ${text}`}>{configuracionPago.pasarelaPago}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className={darkMode ? 'text-gray-300' : 'text-gray-700'}>Moneda:</span>
            <span className={`font-semibold ${text}`}>{configuracionPago.moneda}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className={darkMode ? 'text-gray-300' : 'text-gray-700'}>Comisión:</span>
            <span className={`font-semibold ${text}`}>{configuracionPago.comision}%</span>
          </div>
          <div className="flex justify-between items-center">
            <span className={darkMode ? 'text-gray-300' : 'text-gray-700'}>Estado:</span>
            <span className={`px-2 py-1 rounded text-xs ${
              configuracionPago.activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
            }`}>
              {configuracionPago.activo ? 'Activo' : 'Inactivo'}
            </span>
          </div>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNuevoCurso({...nuevoCurso, estudiantes: e.target.value as any})}
                    placeholder="0"
                    className={`w-full p-2 border ${border} rounded ${darkMode ? 'bg-gray-700' : ''}`}
                  />
                </div>
  ...
  const renderTramites = () => <TramitesSection />;
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <input
                        ...
                        value={(nuevoComponente.posicion as any).left || ''}
                        onChange={(e) => setNuevoComponente({...nuevoComponente, posicion: {...(nuevoComponente.posicion as any), left: e.target.value}})}
                        className={`w-full p-2 border ${border} rounded ${darkMode ? 'bg-gray-600' : ''}`}
                      />
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setGenerarCertificadoPlantilla(null)}>
          <div className={`${card} rounded-lg p-6 max-w-2xl w-full mx-4`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button onClick={() => {
                  const estudiante = (document.getElementById('estudiante-select') as HTMLInputElement).value;
                  const curso = (document.getElementById('curso-select') as HTMLInputElement).value;
                  const codigoQR = generarCodigoQR();
  ...
  const renderActivos = () => <ActivosSection />;

  const renderInventario = () => <InventarioSection />;

  const renderNomina = () => <NominaSection />;

  const renderRecursosHumanos = () => <RecursosHumanosSection />;

  const renderCarnetizacion = () => <CarnetizacionMejorada />;

  const renderBiblioteca = () => <BibliotecaSection darkMode={darkMode} />;

  const renderSolicitudes = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className={`text-2xl font-bold ${text}`}>Solicitudes</h2>
      </div>
      <div className={`${card} rounded-lg shadow overflow-hidden`}>
        <table className="w-full">
          <thead className={`${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
            <tr>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Tipo</th>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Usuario</th>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Asunto</th>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Estado</th>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Fecha</th>
              <th className={`p-3 text-left text-sm font-semibold ${text}`}>Prioridad</th>
            </tr>
          </thead>
          <tbody>
            {solicitudes.map(s => (
              <tr key={s.id} className={`border-t ${border}`}>
                ...
                <td className={`p-3 ${text}`}>{s.usuario}</td>
                <td className={`p-3 ${text}`}>{s.asunto}</td>
                <td className="p-3">
                  <span className={`px-3 py-1 rounded text-xs ${
                ...
                <td className={`p-3 text-sm ${text}`}>{s.fecha}</td>
                <td className="p-3">
                  <span className={`px-3 py-1 rounded text-xs ${
  ...
  const renderContent = () => {
    switch(activeSection) {
      ...
      case 'recursoshumanos': return renderRecursosHumanos();
      case 'carnetizacion': return renderCarnetizacion();
      case 'tramites': return renderTramites();
      case 'anuncios': return <AnunciosSection />;
      case 'mensajesmasivos': return <MensajesMasivosSection />;
      case 'analiticas': return <AnalyticsSection />;
      ...
      <div className={`flex h-screen ${bg}`}>
        <div className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gradient-to-b from-blue-900 to-blue-700 text-white transition-all duration-300 flex flex-col`}>
          <div className="p-4 flex items-center justify-between border-b border-blue-600">
            ...
            <button 
              onClick={() => logout()} 
              className="p-2 hover:bg-red-600 rounded flex items-center gap-2 ml-2" 
              title="Cerrar sesión"
            >
              <LogOut size={20} />
          ...
          <div className={`${card} shadow-sm p-4 flex justify-between items-center`}>
            ...
            <div className="flex-1 max-w-md mx-4 relative" ref={searchRef}>
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Buscar estudiantes, docentes, cursos, activos..."
                  value={searchQuery}
              ...
              {searchOpen && searchResults.length > 0 && (
                <div className={`absolute top-full left-0 right-0 mt-2 ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg border ${
                  ...
                  {searchResults.map((result) => (
                    <div
                      ...
                      className={`p-3 cursor-pointer hover:${darkMode ? 'bg-gray-700' : 'bg-gray-50'} border-b ${
                        darkMode ? 'border-gray-700' : 'border-gray-100'
                      } last:border-b-0 transition`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          result.tipo === 'estudiante' ? 'bg-blue-100 text-blue-600' :
                          result.tipo === 'docente' ? 'bg-green-100 text-green-600' :
                          result.tipo === 'curso' ? 'bg-purple-100 text-purple-600' :
                          result.tipo === 'activo' ? 'bg-orange-100 text-orange-600' :
                          result.tipo === 'empleado' ? 'bg-pink-100 text-pink-600' :
                          'bg-yellow-100 text-yellow-600'
                        }`}>
                          {result.tipo === 'estudiante' && <Users size={20} />}
                          {result.tipo === 'docente' && <Award size={20} />}
                          {result.tipo === 'curso' && <BookOpen size={20} />}
                          {result.tipo === 'activo' && <Building2 size={20} />}
                          {result.tipo === 'empleado' && <User size={20} />}
                          {result.tipo === 'libro' && <BookOpen size={20} />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className={`font-semibold ${text} truncate`}>{result.titulo}</p>
                          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} truncate`}>{result.subtitulo}</p>
                        </div>
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          result.tipo === 'estudiante' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                          result.tipo === 'docente' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                          result.tipo === 'curso' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                          result.tipo === 'activo' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' :
                          result.tipo === 'empleado' ? 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400' :
